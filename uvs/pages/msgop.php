<?php 
   ...
   if(isset($_POST['eno']) && isset($_POST['filiere']) && isset($_POST['envoyer'])) {
        ...
        $numbers=$requ->fetchAll();
        $numbers = array();
        for($i = 0; $i < count($reponse); $i++){
            $numbers[] = $reponse[$i]['numeroTel'];
        }
        $message = $_POST['message'];    
        foreach($numbers as $nume => $numero):
            $url = "https://api.orange.com/smsmessaging/v1/outbound/tel%3A%2B221772357546/requests";
            $ch = curl_init($url);
            $headers = array("Authorization: Bearer gBJIi8MK0BwNPK1WtrJOaJIPcKcC", "Content-Type: application/json");
            ...
            $env = array(
                ...
                //    'address' => 'tel:'.$numero,
                 //   'senderAddress' => 'tel:+221772357546',
                //    'outboundSMSTextMessage' => array(
                //        'message' => $message
                //    )
               // )
            );
            curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($env));
            $output = curl_exec($ch);    
            curl_close($ch);
        endforeach;
    ...
    elseif(isset($_POST['envoyer']) ){//&& $_POST['eno']=='Aucun')){
        if(isset($_POST['envoyer']) && !empty($_POST['message']) && !empty($_POST['numeros'])){
            $numbers = array();
            $nn = $_POST['numeros'];
            $numbers = explode(',', $nn);
            $message = $_POST['message'];
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Campagne SMS (opérateur)</h1>
                                ...
                                <div class="panel panel-yellow">
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <div class="form-group input-group">
                                                ...
                                                <input type="texterea" name="numeros"  id='numeros' class="form-control">
