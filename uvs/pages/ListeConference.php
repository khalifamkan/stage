<?php
    ...
    $heure = date("H:i");
    //$requ = $bdd->prepare('SELECT * FROM Conference where dateC>=:log AND heure>=:log1');
    //$requ->execute(array("log"=>$dateC,"log1"=>$heure));
    $requ = $bdd->prepare('SELECT * FROM Conference');
    $requ->execute();


    if(isset($_POST['envoyer']) && $_POST['groupe']!="aucun" ){
        $req=$bdd->prepare('insert into Participant(nomConf,participant,groupe) values(:nomConf,:participant,:groupe)');
        $req->execute(array(':nomConf'=>$_POST['eno'],':participant'=>'non',':groupe'=>$_POST['groupe'])); 
        $req->closeCursor();
        ?>  
        <script type = "text/javascript">
            res = alert('Groupe Bien enregistré');
        </script>
        <?php
    }
    elseif(isset($_POST['envoyer']) && isset($_POST['user']) && $_POST['user'] !="aucun" ){
        $req=$bdd->prepare('insert into Participant(IdConf,participant,groupe) values(:IdConf,:participant,:groupe)');
        $req->execute(array(':IdConf'=>$_POST['eno'],':participant'=>$_POST['groupe'],':groupe'=>'non')); 
        $req->closeCursor();
        ?>  
        <script type = "text/javascript">
            res = alert('User Bien enregistré');
        </script>
        <?php
    }
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                        <?php $i=1; while($reponse=$requ->fetch()){
                                                    ?>  
                                        <tr>                                                        
                                            <td><?php echo $i; ?></td>
                                            <td><?php echo ($reponse["nom"]); ?></td>
                                            <td><?php echo ($reponse["theme"]); ?></td>
                                            <td><?php echo ($reponse["description"]); ?></td>
                                            <td><?php echo ($reponse["dateC"]); ?></td>
                                            <td><?php echo ($reponse["heure"]); ?></td>
                                        </tr>
                                        <?php $i = $i + 1;} ?>
