...
if ($diff > 0) {
    //echo "conference termine,il y'a ".$diff;
    //echo "\n date de la conference ".$_GET['d'];
    //echo "\n date du systeme ".date("Y-m-d");
    $interval = date_diff($datetime1, $datetime2);
...
elseif ($diff < 0) {
    ...
    //echo "conference demarre dans :";
    $heures   = $heuresConf[0] - date("H");  // les heures < 24
    $minutes  = $heuresConf[1] - date("i") ;   // les minutes  < 60
    $secondes = 00;  // les secondes  < 60
    $annee = $dateConf[0] - date("Y"); // par defaut cette année
    $mois  = $dateConf[1] - date("m");
    $jour  = $dateConf[2] - date("d");
    //$redirection = 'http://10.149.0.176/uvs/pages/VisioConfBisold.php?d='.$_GET['d'].'&h='.$_GET['h'].'&r='.$_GET['r']; // quand le compteur arrive à 0
    $redirection = 'https://10.149.0.176:7443/ofmeet/?r='.$_GET['r'];
                                ...
                                <div class="panel panel-yellow">
                                    ...
                                    <body onload="timer">
                                        ...
                                        <?php
                                            ...
                                            <span style="font-size: 36px;">La conférence est terminée il y a :</span>
                                            <span style="font-size: 36px;"> <?php echo $diff.' jour(s)';?> </span>
                                            <?php
                                         }elseif ($reponse == 2 ) {?>
                                            <span style="font-size: 36px;">La conférence aura dans :</span>
                                            <span style="font-size: 36px;"> <?php echo 'Jour J '.$diff;?> </span>
                                            <?php
                                        }elseif ($reponse == 3 ) {?>
                                            <span style="font-size: 36px;">La conférence a déja démarré</span>
                                            <?php }  ?>
